...
#	 until the while statement is False or until value of 'i' reaches 15 that is
    ...
    then # if match, jump to the while loop below
        while [[ $i -ge 1 && $i -le 15 ]] # loop while value of 'i' is between 1 and 15 -->ref: stackoverflow.com        
    ...
    then
        for i in {1..15}
        do
            #script does not support floating point arithmetic ref: https://linuxhint.com
	...
	echo "..."
    fi #end of fncalculator() function
}
...
#User interaction -  error message and remind user what the acceptabnle range is
  echo "Invalid input, number must be in range (1-15)."
  fnentryvalid #reuse validating function without program exiting
fi # end of program
